...
export const mergeResults = ({
  ...
  const retryDiffById = new Map<
    ...
    ScreenshotDiffResultWithBaseReplayId
  >();
  const flattenedHeadScreenShotDiffResults = flattenScreenshotDiffResults(
    comparisonToHeadReplay
  );

  flattenedHeadScreenShotDiffResults.forEach((result) => {
  ...
  const flattenedCurrentResult = flattenScreenshotDiffResults(currentResult);

  const newScreenshotDiffResults = flattenedCurrentResult.map(
    (currentDiff): ScreenshotDiffResultWithBaseReplayId => {
