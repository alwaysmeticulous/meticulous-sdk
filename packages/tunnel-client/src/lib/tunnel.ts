...
export class Tunnel extends (EventEmitter as new () => TypedEmitter<TunnelEvents>) {
  ...
  async _establish(info: TunnelInfo) {
    ...
    const agentType = "http2-multiplexing-workers";
    ...
    // emit the url immediately since workers are handling the connections
    this.emit("url", {
      url: info.url,
  ...
  async _establishMultiplexingCluster({
    ...
    this.logger.debug(
      "establishing tunnel %s://%s:%s <> %s:%s using worker threads",
      localProtocol,
      localHost,
      localPort,
      remoteHost,
    ...
    this.logger.debug(`Started ${numWorkers} worker threads for tunnel`);
  }
